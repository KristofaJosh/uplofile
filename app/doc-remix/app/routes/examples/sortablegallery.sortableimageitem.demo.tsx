...
export function SortableImageItem({ item }: { item: UploadFileItem }) {
      ...
      {item.status === "done" && (
        <>
          ...
          <div
            {...attributes}
            {...listeners}
            className="absolute top-2 left-2 bg-white/80 backdrop-blur-sm text-gray-600 p-1 rounded-md shadow-sm border opacity-0 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing"
          >
            <IoReorderTwoOutline className="h-3 w-3" />
          </div>
        </>
      )}
